...
void map_insert(Map map, Pointer key, Pointer value) {
	...
	if (simple_insert)
		...
		uint start_pos = (map->hash_function(key) + NEIGHBOURS) % map->capacity;
		bool displaced_success;

		while (start_pos < pos){
			displaced_success = false;
			for (int i = 0; i < NEIGHBOURS; i++){ //swap
				int displaced = pos - NEIGHBOURS + i;
				MapNode displaced_node = &map->array[displaced];
				int displaced_hash = map->hash_function(displaced_node->key) % map->capacity;
				if (displaced_hash + NEIGHBOURS >= pos){

					node = &map->array[pos];
					Pointer temp1 = node->key;
					Pointer temp2 = node->value;
					State temp3 = node->state;
					node->key = displaced_node->key;
					node->value = displaced_node->value;
					node->state = displaced_node->state;
					displaced_node->key = temp1;
					displaced_node->value = temp2;
					displaced_node->state = temp3;					
...
MapNode map_find_node(Map map, Pointer key) {
	...
	for (uint pos = map->hash_function(key) % map->capacity;		// ξεκινώντας από τη θέση που κάνει hash το key
		count <= NEIGHBOURS;							// αν φτάσουμε σε EMPTY σταματάμε
		pos = (pos + 1) % map->capacity, count++) {						// linear probing, γυρνώντας στην αρχή όταν φτάσουμε στη τέλος του πίνακα
